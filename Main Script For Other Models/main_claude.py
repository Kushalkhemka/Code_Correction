...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('claude_analysis.log'),
...
logger.info(f"Claude API key loaded: {anthropic_api_key[:15]}...{anthropic_api_key[-4:]} (length: {len(anthropic_api_key)})")

BASE_DIR = Path(os.path.abspath(os.path.dirname(__file__)))
PYTHON_PROGRAMS_DIR = BASE_DIR / "python_programs"
FIXED_PROGRAMS_DIR = BASE_DIR / "fixed_programs_claude"
JSON_TESTCASES_DIR = BASE_DIR / "json_testcases"
PYTHON_TESTCASES_DIR = BASE_DIR / "python_testcases"
RESULTS_DIR = BASE_DIR / "claude_results"
...
if PYTHON_PROGRAMS_DIR.exists():
    ...
    logger.info(f"Found {len(available_programs)} programs to analyze with Claude")
...
try:
    ...
    claude_model = OpenAIChatCompletionsModel(model="claude-3-7-sonnet-latest", openai_client=claude_client)
...
bug_finder_agent = Agent(
    name="ClaudeBugClassifier",
    instructions=f"""
    You are Claude 3.7 Sonnet, a Bug Classifier agent specialized in analyzing Python code and classifying bugs.
...
bug_fixer_agent = Agent(
    name="ClaudeBugFixer",
    instructions=f"""
    You are Claude 3.7 Sonnet, a Bug Fixer agent specialized in fixing Python code bugs with proper classification.
    ...
    BUG FIX COMPLETE (Claude 3.7 Sonnet):
...
async def analyze_single_program(program_name: str) -> dict:
    """Analyze a single program with Claude 3.7 Sonnet."""
...
async def run_batch_analysis(programs_to_analyze=None):
    """Run analysis on multiple programs with Claude 3.7 Sonnet."""
    if programs_to_analyze is None:
        programs_to_analyze = available_programs
    
    logger.info(f"Starting Claude 3.7 Sonnet batch analysis of {len(programs_to_analyze)} programs")
    ...
    for coro in asyncio.as_completed(tasks):
        ...
        print(f"Claude Progress: {completed}/{total} ({progress:.1f}%) - {result['program']} ({result['status']}){bug_info}")
    ...
    summary_file = RESULTS_DIR / f"claude_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(summary_file, "w") as f:
        json.dump(summary, f, indent=2)
    
    print(f"\n{'='*60}")
    print(f"CLAUDE 3.7 SONNET ANALYSIS COMPLETE")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    logger.info(f"Selected programs for Claude analysis: {len(programs_to_run)} total")
    
    try:
        asyncio.run(run_batch_analysis(programs_to_run))
    except KeyboardInterrupt:
        logger.info("Claude analysis interrupted by user")
    except Exception as e:
