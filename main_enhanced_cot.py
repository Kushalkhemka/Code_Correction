...
async def test_code_verbose(context, arguments) -> str:
    ...
    try:
        ...
        if show_details and result.get("failed_tests"):
            # Load test cases to show actual inputs/outputs
            ...
            if json_file.exists():
                ...
                # Enhance failed test details with actual inputs
                enhanced_failures = []
        ...
        # If comparison requested, also run on other modes
        if compare_versions:
...
code_tester_tool = FunctionTool(
    name="test_code_verbose",
    description="Tests code using NativeTestRunner with detailed verbose output and failure analysis",
    params_json_schema={
        "type": "object",
        "properties": {
...
# Enhanced agents with CoT, Self-Refine, and ReAct prompting
...
bug_fixer_agent = Agent(
    ...
    CRITICAL RULES:
    - If this is iteration 2+, ANALYZE why previous fixes failed
    - Don't repeat the same fix if tests are still failing
    - Look for DIFFERENT algorithmic issues if the same tests keep failing
    - Previous failure feedback contains crucial information - use it!
...
test_evaluator_agent = Agent(
    ...
    """,
    model="o3-mini",
    tools=[code_tester_tool]
)

# Enhanced main function with better iteration logic
async def analyze_and_fix_program(program_name, max_iterations=3):
    ...
    print(f"\n=== Deep Analysis of program: {program_name} ===")
    ...
    # Step 1: Deep bug analysis using CoT
    print(f"\n--- Phase 1: Deep Bug Analysis (Chain of Thought) ---")
    bug_result = await Runner.run(
        bug_finder_agent, 
        f"Perform a deep Chain of Thought analysis of the '{program_name}' program. Read the code, understand the algorithm, trace execution, and identify the root cause of any bugs."
    )
    bug_analysis = bug_result.final_output
    print(f"Bug Analysis Result:")
    print(bug_analysis)
    
    # Iterative fixing with enhanced feedback
    iteration = 1
    previous_failures = []
    test_feedback = ""
    
    while iteration <= max_iterations:
        print(f"\n--- Phase 2: Iteration {iteration}/{max_iterations} (Self-Refine + ReAct) ---")
        
        # Step 2: Fix using Self-Refine methodology
        print(f"Running Self-Refine bug fixing...")
        fix_prompt = f"""
        Use Self-Refine methodology to fix the '{program_name}' program.
        
        Initial Bug Analysis: {bug_analysis}
        ...
        fix_result = await Runner.run(bug_fixer_agent, fix_prompt)
        fix_result_output = fix_result.final_output
        print(f"Fix Implementation:")
        print(fix_result_output)
        
        # Step 3: Comprehensive testing using ReAct
        print(f"Running comprehensive ReAct test evaluation...")
        ...
        test_evaluation = test_result.final_output
        print(f"Test Evaluation:")
        print(test_evaluation)
        
        # Step 4: Parse results and decide next steps
        try:
            eval_data = json.loads(test_evaluation)
            ...
            if validation_passed:
                ...
                print(f"\nIteration {iteration}: Tests failed with {pass_rate:.1%} pass rate")
...
CRITICAL: If the same tests keep failing, you need to find DIFFERENT bugs or use a COMPLETELY DIFFERENT approach. The current algorithmic approach may be fundamentally flawed.
"""
                else:
                    print(f"FAILED: Could not fix program after {max_iterations} iterations")
                    return {
                        "program": program_name,
                        "success": False,
                        "iterations": max_iterations,
                        "bug_analysis": bug_analysis,
                        "final_fix_result": fix_result_output,
                        "final_test_evaluation": test_evaluation,
                        "failure_history": previous_failures
                    }
        except json.JSONDecodeError as e:
            print(f"Warning: Could not parse test evaluation JSON: {e}")
            ...
            if iteration >= max_iterations:
                ...
                test_feedback = f"Previous test evaluation could not be parsed (iteration {iteration}). Please ensure your fix is correct and provide valid JSON output."
...
if __name__ == "__main__":
    ...
    print(f"Starting enhanced analysis of program: {program_name} with max {max_iterations} iterations")
    print("Using Chain of Thought + Self-Refine + ReAct methodologies")
    
    try:
        loop = asyncio.get_event_loop()
        result = loop.run_until_complete(analyze_and_fix_program(program_name, max_iterations))
        
        if result:
            if result["success"]:
                print(f"\nFINAL RESULT: Successfully fixed {program_name} in {result['iterations']} iterations")
            else:
                print(f"\nFINAL RESULT: Failed to fix {program_name} after {result['iterations']} iterations")
                print("Failure history:")
                for failure in result.get("failure_history", []):
